...
import { FormControl, Form, Accordion, Card, Col, Button, OverlayTrigger, Tooltip, Spinner } from 'react-bootstrap';
...
const Mission = (props) => {
	...
	const [ position, setPosition ] = useState(calcPosition(props.selectedElement.position.y));
	const [ completedBy, setCompletedBy ] = useState(props.selectedElement.data.completedBy);
	...
	const [ selectedSeries, setSelectedSeries ] = useState(props.selectedElement.data.selectedSeries);
		...
		() => {
			...
			missionsCopy.map((el) => {
				if (el.id === props.selectedElement.id) {
					...
					let incomers = getIncomers(props.selectedElement, props.missionTree.missions);
					let incomerNames = incomers.map((incomer) => {
						return incomer.data.label;
					});
					let date = year + '.' + month + '.' + day;
					if (year === undefined || month === undefined || day === undefined) date = '';
					el.data = {
						...el.data,
						label: nodeName,
						icon: icon,
					...
					if (selectedSeries != undefined && selectedSeries != null && selectedSeries != 'Select Series') {
						el.style = {
							...el.style,
	...
	return (
		<Form>
			<Form.Group controlId="formSeries">
				...
				<Form.Text className="text-muted">(Required) A mission needs a Series</Form.Text>
			</Form.Group>
			...
			<Form.Group controlId="formIcon">
				...
				/>
				<Form.Text className="text-muted">The icon to use for the mission</Form.Text>
			</Form.Group>
			...
			<Form.Group controlId="formGeneric">
				...
				/>
				<Form.Text className="text-muted">Whether this mission is considered generic.</Form.Text>
			</Form.Group>
			...
			<Form.Group controlId="formPosition">
				...
				<FormControl
					...
					style={{ width: '20%' }}
				/>
				<Form.Text className="text-muted">Which row the mission appears in. 1 is top.</Form.Text>
			</Form.Group>

			<Form.Row style={{ width: '100%' }}>
				<Form.Group as={Col} controlId="formYear" style={{ minWidth: '35%', width: '35%' }}>
					...
					/>
					<Form.Text className="text-muted" style={{ minWidth: '50%', width: '50%' }}>
						Year mission is automatically completed.
					</Form.Text>
				</Form.Group>

				<Form.Group as={Col} controlId="formMonth" style={{ minWidth: '25%', width: '25%' }}>
					...
					/>
					<Form.Text className="text-muted" style={{ minWidth: '40%', width: '40%' }}>
						Month mission is automatically completed.
					</Form.Text>
				</Form.Group>
				<Form.Group as={Col} controlId="formDay" style={{ minWidth: '25%', width: '25%' }}>
				...
				<OverlayTrigger
					...
					overlay={<Tooltip id="button-tooltip">{'Remove Completed By'}</Tooltip>}
				>
					<Button
						variant="danger"
			...
			<Form.Group controlId={'formIncomer'}>
				...
				{getIncomers(props.selectedElement, props.missionTree.missions).map((incomer) => (
			...
			<Form.Group controlId="formTrigger">
				...
				/>
				<Form.Text className="text-muted">Determines when the mission is completed. Country scope.</Form.Text>
			</Form.Group>
