...
const MissionTab = (props) => {
	...
	const onElementClick = (event, element) => (console.log('click', element), setSelectedElement(element));
		...
		if (isNode(node) && node.data.selectedSeries != null) {
			...
			elements.map((el) => {
				if (isNode(el) && el.id === node.id) {
					...
					console.log('updating position for seletected element', selectedElement);
				}
			});
			let missionTabsCopy = [ ...props.missionTabs ];
			let index = missionTabsCopy.findIndex((missionTab) => missionTab.id === props.fileID);
